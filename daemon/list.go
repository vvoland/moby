...
func (daemon *Daemon) reduceContainers(ctx context.Context, config *types.ContainerListOptions, reducer containerReducer) ([]*types.Container, error) {
	...
	for i := range containerList {
		t, err := daemon.reducePsContainer(ctx, &containerList[i], lctx, reducer)
		if err != nil {
...
func (daemon *Daemon) reducePsContainer(ctx context.Context, container *container.Snapshot, lctx *listContext, reducer containerReducer) (*types.Container, error) {
	// filter containers to return
	switch includeContainerInList(container, lctx) {
	...
	newC, err := reducer(ctx, container, lctx)
	if err != nil {
		return nil, err
	}
...
func (daemon *Daemon) foldFilter(ctx context.Context, view container.View, config *types.ContainerListOptions) (*listContext, error) {
	...
	if psFilters.Contains("ancestor") {
		...
		psFilters.WalkValues("ancestor", func(ancestor string) error {
			_, img, err := daemon.imageService.GetImage(ctx, ancestor, nil)
			if err != nil {
...
func (daemon *Daemon) refreshImage(ctx context.Context, s *container.Snapshot, lctx *listContext) (*types.Container, error) {
	c := s.Container
	image := s.Image // keep the original ref if still valid (hasn't changed)
	if image != s.ImageID {
		_, img, err := daemon.imageService.GetImage(ctx, image, nil)
