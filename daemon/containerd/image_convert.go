...
func (i *ImageService) Convert(ctx context.Context, src string, dst reference.NamedTagged, opts imagetypes.ConvertOptions) error {
	...
	if _, err := i.images.Get(ctx, dst.String()); !cerrdefs.IsNotFound(err) {
		return errdefs.Conflict(fmt.Errorf("image %s already exists", dst))
	}
	...
	if !images.IsIndexType(srcMediaType) {
		return errdefs.InvalidParameter(errors.New("cannot convert "))
	}
	...
	if n != nil {
		...
		desc, err := storeJson(ctx, i.content, newIndex.MediaType, newIndex, info.Labels)
		if err != nil {
			return errdefs.System(fmt.Errorf("failed to write modified index: %w", err))
		}
