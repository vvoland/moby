	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, all []*imagetypes.Summary) {
				...
				assert.Check(t, is.Equal(all[0].Manifests[0].Kind, imagetypes.ImageManifestKindImage))
			},
		},
		{
			...
			check: func(t *testing.T, all []*imagetypes.Summary) {
				...
				assert.Check(t, is.Equal(i.Manifests[0].Kind, imagetypes.ImageManifestKindImage))
				assert.Check(t, is.Equal(i.Manifests[0].ImageData.Platform.Architecture, "arm64"))
				assert.Check(t, is.Equal(i.Manifests[1].Kind, imagetypes.ImageManifestKindImage))
		...
		},
		{
			...
			check: func(t *testing.T, all []*imagetypes.Summary) {
				...
				assert.Check(t, is.Equal(all[1].Manifests[0].Kind, imagetypes.ImageManifestKindImage))
				assert.Check(t, is.Equal(all[1].Manifests[1].Kind, imagetypes.ImageManifestKindImage))
			},
		},
		{
