...
type ImageExportedByBuildkit interface {
	ImageExportedByBuildkit(ctx context.Context, id string, desc ocispec.Descriptor) error
}
...
func (i *imageExporterInstanceWrapper) Export(ctx context.Context, src *exporter.Source, inlineCache exptypes.InlineCache, sessionID string) (map[string]string, exporter.DescriptorReference, error) {
	...
	if i.callback != nil {
		i.callback.ImageExportedByBuildkit(ctx, imageID, desc)
	}
	return out, ref, nil
}
