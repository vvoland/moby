...
func TestExecSocketDenied(t *testing.T) {
	...
	t.Run("socketcall_int80", func(t *testing.T) {
		...
		t.Run("AF_VSOCK", func(t *testing.T) {
			...
			assert.Assert(t, res.ExitCode == 1, "expected AF_VSOCK connect to fail before listener acceptance; listener result: %v; client output: %s", exchange.err, clientOutput)
			assert.Assert(t, strings.Contains(strings.ToLower(clientOutput), "connect"), "unexpected AF_VSOCK client failure before listener acceptance; listener result: %v; client output: %s", exchange.err, clientOutput)
		})
